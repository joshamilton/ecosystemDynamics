...
#%%#############################################################################
### For each lake, import and subset the OTU table. Perform a persistence
### and abundance sweep.
################################################################################
...
for lake in lakeList:
    ...
    # Subset to just the lake of interest
    lakeColumns = [col for col in otuTable.columns if col[:3]==lake]
    otuTable = otuTable[lakeColumns]
    otuTable = otuTable.loc[~(otuTable==0).all(axis=1)]
    ...
    avgRelAbundTable.to_csv(deblurDir+'/epiRelAbundTable-'+lake+'.csv')
    ...
    # Write to file
    recoveryTable.to_csv(deblurDir+'/persistAbund-TotalRecovery-epi-'+lake+'.csv')
    worstTable.to_csv(deblurDir+'/persistAbund-WorstRecovery-epi-'+lake+'.csv')
    totalOtuTable.to_csv(deblurDir+'/persistAbund-TotalOTUs-epi-'+lake+'.csv')
    ...
    plt.savefig(deblurDir+'/persistAbund-epi-'+lake+'.png')